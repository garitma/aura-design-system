...
export const WithDynamicFormValidator = () => {
  const status = useStatus();
  ...
  const validatorSchema = (fields) => {
    ...
    if (!validator.isEmail(email.value)) {
      email.dialog("Enter a valid mail.");
      return;
    } else {
      email.dialog(null);
    }
    ...
    <div>
      <p>
        <span className="info info-text wall-pad">
          External library <span className="font-bold">$ pnpm i validator</span>
        </span>{" "}
        is a library for string validations.{" "}
        <a

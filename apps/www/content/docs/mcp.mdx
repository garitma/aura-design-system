...
4. Install Aura `@shadcn/lint` policy: `pnpm add -D @shadcn/lint` then `pnpm dlx shadcn@latest add @aura/eslint-shadcn-lint`, and merge `createAuraShadcnLintConfig` into `eslint.config` (see [Agent blueprint](/docs/mcp-agent-blueprint) § D).
5. Paste a component URL, or describe a complete landing page and let the agent plan its required image set.

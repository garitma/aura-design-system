...
4. Install Aura design lint: see [Lint](/docs/lint) (`@aura/eslint-shadcn-lint` + `@shadcn/lint`).
5. Optional agent rigor: [pstack](/docs/pstack) via `/add-plugin pstack` then `/setup-pstack` (blueprint also audits it — [Agent blueprint](/docs/mcp-agent-blueprint) § E). Does not replace Aura rules/lint.
6. Paste a component URL, or describe a complete landing page and let the agent plan its required image set.

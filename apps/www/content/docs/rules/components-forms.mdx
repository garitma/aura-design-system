---
title: "Aura Form System"
description: "Form system — useFormDynamic, Form components, validation (validateFormData), and field composition."
---
...
* **MUST**: Use **FormCheckboxGroup** / **FormRadioGroup** with `field.setValue` (array or string). For composed controls (Select, Combobox, Editor, SignaturePad, SortableList), use `field.setValue(value)` in the control’s change handler; do not pass `field.value` as controlled value for Select (uncontrolled + `onValueChange` → `field.setValue`).

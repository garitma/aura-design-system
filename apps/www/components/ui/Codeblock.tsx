...
export function CodeBlock({
  ...
  return (
    ...
    >
      {title ? (
        <div className="flex text-fd-muted-foreground items-center gap-1 h-3 border-b px-1">
          {typeof icon === "string" ? (
            <div
              className="[&_svg]:size-1"
...
export function CodeBlockTabsList(props: ComponentProps<typeof TabsList>) {
  return (
    <TabsList
      {...props}
      className={cn(
        "flex flex-row px-0.5 overflow-x-auto text-fd-muted-foreground",
        props.className
      )}
    >
      {props.children}
...
export function CodeBlockTabsTrigger({
  ...
  return (
    <TabsTrigger
      {...props}
      className={cn(
        "relative group inline-flex text-sm font-medium text-nowrap items-center transition-colors gap-0.5 px-0.5 py-0.5 hover:text-fd-accent-foreground data-[state=active]:text-fd-primary [&_svg]:size-1",
        props.className
      )}
    >
      <div className="absolute inset-x-0.5 bottom-0 h-px group-data-[state=active]:bg-fd-primary" />
      {children}
    </TabsTrigger>
  );
}

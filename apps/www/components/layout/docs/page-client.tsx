...
export function PageTOCPopoverTrigger(props: ComponentProps<"button">) {
  ...
  return (
    <CollapsibleTrigger
      {...props}
      className={cn(
        "flex w-full h-(--fd-tocnav-height) items-center text-sm text-fd-muted-foreground gap-1 pl-1.5 pr-2 py-1 text-start focus-visible:outline-none [&_svg]:size-1 md:px-2",
        props.className
      )}
    >
      <ProgressCircle
    ...
    >
      <Collapsible open={open} onOpenChange={setOpen} asChild>
        <header
          ...
          {...props}
          className={cn(
            "fixed pr-(--removed-body-scroll-bar-size,0) z-10 border-b transition-colors bg-gray-2 xl:hidden max-xl:on-root:[--fd-tocnav-height:40px]",
            open && "shadow-lg",
            props.className
          )}
          style={{

...
The CLI is the **on-ramp for consumers**: it can create or point at a Next-style app, drop in Aura’s **`components.json`** (registry URLs for production and local docs), regenerate **`globals.css`** from Radix-style accent/gray/background inputs, and pull first-party registry items such as `class-names`, `design-md`, and `rules`.
...
- **`aura init`** — Runs **`pnpm dlx create-next-app@latest`** with inherited stdio (interactive). Detects a newly created app subdirectory or uses the current folder, then:
  ...
  - Runs **`pnpm dlx shadcn@latest add`** for `@aura/class-names`, `@aura/page-get-starter`, `@aura/css-main`, `@aura/design-md`, `@aura/rules`.
